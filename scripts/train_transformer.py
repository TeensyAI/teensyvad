...
def main() -> None:
    ...
    if ckpt.exists():
        ...
        if isinstance(blob, dict) and "model" in blob:
            ...
            done = blob.get("done", 0)
        else:                       # legacy: raw state_dict, position unknown
    ...
    next_val = ((done // args.batches_per_epoch) + 1) * args.batches_per_epoch
    next_ckpt = ((done // 500) + 1) * 500
    print(f"training gru{args.hidden} chunk={args.chunk} … "
          f"{total_target - done:,} batches to go", flush=True)
    tot, since = 0.0, 0
    while done < total_target:
        ...
        if done == next_ckpt:
            # crash-resilient: persist weights + position every 500 batches
